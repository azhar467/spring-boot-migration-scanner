...
def render_html(scan, ruleset_name, output_path, parent_row=None, meta=None):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(_TEMPLATE.format(
            ...
            intro=intro, legend=_LEGEND, parent=parent_html,
            services_label=("<div class='section-label'>Services (%d)</div>" % total) if parent_row else "",
            rows="\n".join(rows)))
    return output_path


# What each rule implies as a next action -- shown inline so the report is
# legible without the author present.
RULE_ACTION = {
    "opensearch-rhlc-usage": "Migrate to the new ES/OpenSearch client before upgrading; OpenRewrite will NOT fix this.",
    "spring-data-es-dep": "Decide client strategy; confirm OpenSearch 1.3 compatibility on Boot 3.x.",
    "legacy-es-rest-dep": "Replace the legacy high-level REST client before upgrading.",
    "javax-persistence": "OpenRewrite can auto-migrate these to jakarta.persistence.",
    "javax-servlet": "OpenRewrite can auto-migrate these to jakarta.servlet.",
    "javax-validation": "OpenRewrite can auto-migrate these to jakarta.validation.",
    "javax-annotation": "OpenRewrite can auto-migrate these to jakarta.annotation.",
    "parent-boot-version": "Service is still on Boot 2.x; this is a migration target.",
    "redis-deprecated-keys": "Rename properties to spring.data.redis.* .",
    "deprecated-security-adapter": "Rewrite security config to SecurityFilterChain (manual).",
    "deprecated-batch-properties": "Rename to spring.batch.jdbc.initialize-schema .",
}
...
def _repo_block(repo, is_parent=False):
    ...
    for f in sorted(repo["findings"], key=lambda x: SEV_ORDER.get(x["severity"], 9)):
        ...
        action = RULE_ACTION.get(f["rule_id"], "")
        action_html = ("<div class='action'>&rarr; %s</div>" % html.escape(action)) \
            if (action and f["status"] != "pass") else ""
        finding_rows.append(
            ...
            "<td class='detail'>{detail}</td></tr>".format(
                ...
                name=html.escape(f["rule_name"]), action=action_html,
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  .action {{ color:var(--accent); font-size:.78rem; margin-top:3px; }}
