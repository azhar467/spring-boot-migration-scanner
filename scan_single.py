...
self-contained HTML readiness dashboard (plus optional CSV/JSON).
...
Usage:
    python3 scan_single.py --config config.yaml
    python3 scan_single.py --config config.yaml --token $GITLAB_TOKEN --csv --json
    python3 scan_single.py --dry-run            # scans bundled DRY_RUN_SAMPLES

Config: either point --config at a tiny YAML/JSON file, or just edit the CONFIG
dict below and run with no --config at all.
"""
...
CONFIG = {
    "gitlab_url": "https://gitlab.your-lfg-host.example.com",
    "default_ref": "main",                       # master/develop if needed
    "parent_pom_project": "platform/parent-pom",  # ID or group/path
    ...
    "projects": [
        ...
        # ... add the remaining services (id required, name optional, ref optional)
    ],
}
...
def run_scan(cfg, token, dry_run, csv_flag, json_flag, base_dir):
    ...
    if not dry_run:
        if not token:
            print("ERROR: no token. Set GITLAB_TOKEN or pass --token, or use --dry-run.",
                  file=sys.stderr)
            sys.exit(2)
    ...
    for repo in cfg["projects"]:
        if isinstance(repo, dict):
            ...
            ref = repo.get("ref") or cfg.get("default_ref", "main")
        else:
            pid = label = str(repo)
            ref = cfg.get("default_ref", "main")
...
def main():
    ...
    if args.config:
        ...
        cfg = CONFIG
        base_dir = os.getcwd()
    run_scan(cfg, args.token, args.dry_run, args.csv, args.json, base_dir)
